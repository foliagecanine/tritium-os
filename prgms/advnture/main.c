...
char *commands[NUM_COMMANDS] = {"help",
                                "exit",
                                "room",
                                "inv",
...
char *helptext[NUM_COMMANDS] = {"Shows help",
                                "Exits program",
...
char *cr_texts[3][3] = {
    ...
    },
    {
        ...
        "center says \"NO TRESSPASSING\"",
        "You are in a stone room with a door to the west. It's rather dark in here.\nMaybe it's because there's no "
        "torch in the torch holder.",
    },
    {
        "You are in a stone room with a doorway to the north.\nThere is a large pressure plate in the center of the "
        "room. You avoid it.",
        ...
        "won the game! Type \"exit\" to exit.",
    }};
...
bool has(uint8_t iid)
{
    ...
    {
        if (inventory[i] == iid) return true;
    }
    return false;
}
...
uint8_t run()
{
    ...
    else if (cr_x == 1 && cr_y == 2)
    {
        ...
        if (!strcmp(cmd, "take torch") && near(2))
        {
            ...
            cr_texts[cr_x][cr_y] = "You are in a stone room with a door to the west. It's rather dark in here.\nMaybe "
                                   "it's because there's no torch in the torch holder.";
            doors[3] = false;
            give_item(2);
        }
    }
    else if (cr_x == 1 && cr_y == 1)
    {
        ...
        if (!strcmp(cmd, "s") || !strcmp(cmd, "south") || !strcmp(cmd, "west") || !strcmp(cmd, "w") ||
            ((!strcmp(cmd, "east") || !strcmp(cmd, "e")) && !has(3)))
        {
        ...
        {
            printf("You find a piece of paper that says \"With this permit in your hand, head to where the sun "
                   "rises.\"\n");
            give_item(3);
        }
    }
    ...
    else if (cr_x == 0 && cr_y == 1)
    {
        ...
        if (!strcmp(cmd, "take torch") && items[2].x == cr_x && items[2].y == cr_y)
        {
            ...
            cr_texts[cr_x][cr_y] = "You are in a stone room with a door to the north and the south. It's rather dark "
...
void main()
{
    ...
    while (run()) current_room_text = cr_texts[cr_x][cr_y];
    exit(0);
}
