...
char *options[TOTAL_ARGS] = {
    "--help", "-h", "-?", "--version", "-v", "-o", "-p",
};

int get_option_index(char *arg)
{
    for (int i = 0; i < TOTAL_ARGS; i++)
        if (!strcmp(arg, options[i])) return i;
    return -1;
}
...
int main(int argc, char **argv)
{
    ...
    if (outputfile == NULL) outputfile = default_output(inputfile);
    ...
    if (retval = encode_ops(in_data, encoded_ops)) return retval;
