...
uint64_t f16_getLocationFromCluster(uint32_t clusterNum, FAT16_MOUNT fm) { return (uint64_t)((clusterNum - 2) * (512 * fm.SectorsPerCluster)) + (fm.NumRootDirectoryEntries * 32) + (fm.RootDirectoryOffset * 512); }
...
void f16_setClusterValue(uint8_t *FAT, uint32_t cluster, uint16_t value) { ((uint16_t *)FAT)[cluster] = value; }
...
uint8_t f16_wipecluster(uint32_t clusterNum, FAT16_MOUNT fm, uint32_t drive_num) {
	...
	for (uint8_t i = 0; i < fm.SectorsPerCluster; i++) {
		uint8_t r = disk_write_sectors(drive_num, (f16_getLocationFromCluster(clusterNum, fm) + (i * 512)) / 512, 1, (uint8_t *)read);
		if (r)
			return r + 8;
	}
	return SUCCESS;
...
// error or success
FILE FAT16_fopen(uint32_t location, uint32_t numEntries, char *filename, uint32_t drive_num, FAT16_MOUNT fm, uint8_t mode) {
	FILE  retFile;
	char  searchname[13];
...
uint8_t FAT16_fread(FILE *file, char *buf, uint32_t start, uint32_t len, uint32_t drive_num) {
	...
	for (uint8_t i = 0; i < fm.FATSize; i++)
		disk_read_sectors(drive_num, i + fm.FATOffset, 1, &FAT[i * 512]);
...
FILE FAT16_fcreate(char *name, FAT16_MOUNT fm, uint32_t drive_num) {
	...
	for (uint8_t i = 0; i < 16; i++) {
		...
		if (!dir_entry->Filename[0]) {
			...
			for (uint8_t i = 0; i < fm.FATSize; i++) {
				if (disk_write_sectors(drive_num, i + fm.FATOffset, 1, &FAT[i * 512]))
					retfile.valid = false;
			}
			break;
		}
