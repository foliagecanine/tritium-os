...
void LongToShortFilename(char *longfn, char *shortfn)
{
    ...
    else
    {
        ...
        { // If there is no dot then just copy the whole thing (up to 8).
    ...
    if (locOfDot != -1)
    {
        ...
        if (extLen > 0) shortfn[8] = longfn[locOfDot + 1];

        if (extLen > 1 && extLen < 4) shortfn[9] = longfn[locOfDot + 2];

        if (extLen > 2 && extLen < 4) shortfn[10] = longfn[locOfDot + 3];
