...
FSMOUNT MountFAT12(uint32_t drive_num)
{
    ...
    fat12mount->SystemAreaSize          = bpb.NumReservedSectors + (bpb.NumFATs * bpb.NumSectorsPerFAT +
...
FILE FAT12_fopen(uint32_t location, uint32_t numEntries, char *filename, uint32_t drive_num, FAT12_MOUNT fm,
                 uint8_t mode)
{
    FILE  retFile;
    char  searchname[13];
