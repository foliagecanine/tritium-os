...
typedef struct {
	...
	bool (*exists_function)(uint16_t drive_num);
} disk_handler;
...
uint8_t register_disk_handler(uint8_t (*read_function)(uint16_t, uint64_t, uint32_t, void *), uint8_t (*write_function)(uint16_t, uint64_t, uint32_t, void *), bool (*exists_function)(uint16_t), uint32_t max_port_count) {
	for (uint16_t i = 0; i < 255; i++) {
		if (!handlers[i].used) {
			...
			handlers[i].exists_function = exists_function;
			return i;
		}
	}
...
void unregister_disk_handler(uint8_t handler_id) {
	...
	handlers[handler_id].exists_function = NULL;
}
...
bool disk_drive_exists(uint32_t drive_num) {
	uint8_t handler = (uint8_t)(drive_num >> 16);
	if (handler >= 255 || !handlers[handler].used)
		return false;
	return handlers[handler].exists_function(drive_num & 0xFFFF);
}
