...
#include <fs/disk.h> //ATA Driver: init_ata, init_ahci
	...
	for (uint16_t i = 0; i < 256; i++) {
		...
		if (c_pci.vendorID!=0xFFFF) {
			for (uint8_t j = 0; j < 32; j++) {
				...
				if (c_pci.vendorID!=0xFFFF) {
					printf("%#:%#.%d [%#:%#] (%#.%#) 0:%# 1:%# 2:%# 3:%# 4:%# 5:%#\n",(uint64_t)i,(uint64_t)j,(uint32_t)0,(uint64_t)c_pci.vendorID,(uint64_t)c_pci.deviceID,(uint64_t)c_pci.classCode,(uint64_t)c_pci.subclass,(uint64_t)c_pci.BAR0,(uint64_t)c_pci.BAR1,(uint64_t)c_pci.BAR2,(uint64_t)c_pci.BAR3,(uint64_t)c_pci.BAR4,(uint64_t)c_pci.BAR5);
					for (uint8_t k = 1; k < 8; k++) {
						pci_t pci = getPCIData(i,j,k);
						if (pci.vendorID!=0xFFFF) {
							printf("    %d [%#:%#] (%#.%#) 0:%# 1:%# 2:%# 3:%# 4:%# 5:%#\n",(uint32_t)k,(uint64_t)pci.vendorID,(uint64_t)pci.deviceID,(uint64_t)pci.classCode,(uint64_t)pci.subclass,(uint64_t)pci.BAR0,(uint64_t)pci.BAR1,(uint64_t)pci.BAR2,(uint64_t)pci.BAR3,(uint64_t)pci.BAR4,(uint64_t)pci.BAR5);
						}
					}
				}
			}
