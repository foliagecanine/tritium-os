...
bool check_command(char *command)
{
    ...
    if (!strcmp(command, "pci"))
    {
        for (uint16_t i = 0; i < 256; i++)
        {
            for (uint8_t j = 0; j < 32; j++)
            {
                ...
                {
                    printf("PCI %u.%u: V%X D%X Class %X Subclass %X PIF %X IRQ %u\n", i, j, pcidevice.vendorID,
                           pcidevice.deviceID, pcidevice.class, pcidevice.subclass, pcidevice.progIF, pcidevice.irq);
                        ...
                        for (uint8_t k = 1; k < 8; k++)
                        {
                            ...
                            if (pcidevice.vendorID != 0xFFFF)
                                printf("    PCI %u.%u.%u: V%X D%X Class %X Subclass %X PIF %X IRQ %u\n", i, j, k,
                                       pcidevice.vendorID, pcidevice.deviceID, pcidevice.class, pcidevice.subclass,
                                       pcidevice.progIF, pcidevice.irq);
                        }
                    }
                }
            }
    ...
    if (!memcmp(command, "pci-dump ", strlen("pci-dump ")))
    {
        ...
        pci_write_config_dword(command[9] - '0', command[10] - '0', command[11] - '0', 4,
                               pci_read_config_dword(command[9] - '0', command[10] - '0', command[11] - '0', 4) |
                                   7); // Enable Bus Mastering, IO space, and Mem IO
        cmdAck = true;
    }
    ...
    if (!strcmp(command, "ls") || !strcmp(command, "dir"))
    {
        ...
        {
            FAT12_print_folder((uint32_t)cd.location, (uint32_t)cd.size,
                               get_disk_mount(tolower(currentDirectory[0]) - 'a').drive);
        }
        cmdAck = true;
    }
