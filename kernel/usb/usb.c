...
void check_pci(pci_t pci, uint8_t i, uint8_t j, uint8_t k)
{
    ...
    if (pci.vendorID != 0xFFFF && pci.class == 0xC && pci.subclass == 3)
    {
        ...
        if (!strcmp(name, "UHCI"))
        {
            ...
            if (rval)
            {
                printf("[USB ] Initialized UHCI controller ID %u with %u ports IRQ %u.\n", (uint8_t)rval - 1,
                       get_uhci_controller(rval - 1)->num_ports, pci.irq);
        ...
        else if (!strcmp(name, "xHCI"))
        {
            ...
            if (rval)
            {
                printf("[USB ] Initialized xHCI controller ID %u with %u ports IRQ %u.\n", (uint8_t)rval - 1,
                       get_xhci_controller(rval - 1)->num_ports, pci.irq);
        ...
        for (uint8_t dev_num = 1; dev_num < 128; dev_num++)
        {
            ...
            if (!usb_device_from_addr(devaddr)->valid) continue;
            usb_dev_desc devdesc = usb_get_dev_desc(devaddr);
            if (!devdesc.length) continue;
...
usb_endpoint_desc usb_get_endpoint_desc(uint16_t dev_addr, uint8_t config_index, uint8_t interface_index,
...
void *usb_create_interval_in(uint16_t dev_addr, void *out, uint8_t interval, uint8_t endpoint_addr,
                             uint16_t max_pkt_size, uint16_t size)
{
    usb_device *device = usb_device_from_addr(dev_addr);
    if (device->valid)
    {
